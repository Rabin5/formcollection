...
from forms.models import (
    CovidHospitalManagementChecklist,
    CovidHospitalManagementChecklistLine
)
from forms.forms.cov_hos_management_checklist_forms import (
    CovidHospitalManagementChecklistForm,
    CovidHospitalManagementChecklistLineFormSet
)
...
class CovidHospitalManagementChecklistUpdateView(UpdateView):
    ...
    def get_context_data(self, **kwargs):
        data = super(CovidHospitalManagementChecklistUpdateView, self).\
            get_context_data(**kwargs)
        if self.request.POST:
            data['lines'] = CovidHospitalManagementChecklistLineFormSet(
                self.request.POST, instance=self.object)
        else:
            cov_checklist = CovidHospitalManagementChecklistDescription.\
                objects.all()
            for cov in cov_checklist:
                get_checklist, checklist = \
                    CovidHospitalManagementChecklistLine.objects.\
                    get_or_create(
                        cov_hos_management=self.object,
                        description=cov
                        )
                get_checklist.save()
            data['lines'] = CovidHospitalManagementChecklistLineFormSet(
