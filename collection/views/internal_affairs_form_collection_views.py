...
class InternalAffairFormCollectionCreateView(View):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        """
        form_collect = InternalAffairFormCollection(
            user=request.user, status='started', state=0)
        form_collect.save()
        self.object = form_collect
        self.init_forms()
        form_url = f"{reverse('internal_affairs_forms:internal_affairs_update', kwargs={'pk': form_collect.pk})}?form={DICT_INTERNAL_AFFAIRS_STATE.get(0)}"
        context = {'url': form_url}
        return JsonResponse(context, content_type='application/json')


class InternalAffairFormCollectionUpdateView(UpdateView):
    """
    ...
    def _get_cur_form_instance(self, pk):
        ...
        """
        collection = InternalAffairFormCollection.objects.get(pk=pk)
    ...
    def _get_metadata(self):
        ...
        """
        total_forms = len(LIST_INTERNAL_AFFAIRS_STATE)
        if self.object.status in ('started', 'incomplete'):
            current_form = LIST_INTERNAL_AFFAIRS_STATE.index(self.current_form)
        else:
            current_form = total_forms
        percentage = int(current_form / total_forms * 100)

        if self.current_form == LIST_INTERNAL_AFFAIRS_STATE[0]:
            self.is_first_form = True

        if self.current_form == LIST_INTERNAL_AFFAIRS_STATE[-1]:
    ...
    def _get_state(self):
        ...
        """
        current_state = LIST_INTERNAL_AFFAIRS_STATE.index(self.current_form)
        if self.next_state == 'previous':
            delta_idx = -1  # step for next form index
        else:
            delta_idx = 1
        next_state = current_state + delta_idx
        self.next_form = DICT_INTERNAL_AFFAIRS_STATE.get(next_state)
        if next_state > len(LIST_INTERNAL_AFFAIRS_STATE)-1:
            next_state = len(LIST_INTERNAL_AFFAIRS_STATE) - 1
        elif next_state < 0:
            next_state = 0
...
class InternalAffairFormCollectionListView(ListView):
    model = InternalAffairFormCollection
